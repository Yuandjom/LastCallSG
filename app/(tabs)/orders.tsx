import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Image, Modal, Animated, Easing } from 'react-native';
...
const Orders = () => {
    ...
    const handleConfirm = () => {
        setClosedOrders([...closedOrders, { ...selectedOrder, status: 'Order completed' }]);
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.tabContainer}>
                ...
                <TouchableOpacity onPress={() => setActiveTab('Closed')} style={styles.tab(activeTab === 'Closed')}>
                    ...
                    openOrders.length > 0 ? (
                        openOrders.map((order) => (
                            <View key={order.id} style={styles.orderContainer}>
                                ...
                                <TouchableOpacity style={styles.redeemButton} onPress={() => handleRedeemPress(order)}>
                        ...
                        closedOrders.map((order) => (
                            <View key={order.id} style={styles.orderContainer}>
                                ...
                                <View style={styles.itemContainer}>
                                    <Text style={styles.totalItems}>Total {order.items.reduce((acc: any, item: { quantity: any; }) => acc + item.quantity, 0)} Items</Text>
                                    <Text style={styles.totalPrice}>S${order.items.reduce((acc: any, item: { price: any; }) => acc + item.price, 0).toFixed(2)}</Text>
                                </View>
                ...
                >
                    <Animated.View style={[styles.modalBackground, { opacity: fadeAnim }]}>
                        <Animated.View style={[styles.modalContent, { transform: [{ translateY: slideAnim }] }]}>
                            ...
                            <View style={styles.collectTimeContainer}>
                                <View style={styles.collectTimeLine} />
                                <Text style={styles.collectTime}>Confirm only when you are at the store and ready for collection!</Text>
                            </View>
